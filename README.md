...
- `oneapi_host()` reports the installed Intel oneAPI toolkit, read from the component layout under `$ONEAPI_ROOT` (falling back to `/opt/intel/oneapi`). It covers the **toolkit**, not the GPU runtime: a host using a distro-packaged Level Zero driver with no toolkit reports `None`, since reading that runtime's version needs linking rather than a file read. **Not yet verified against a real install.**
